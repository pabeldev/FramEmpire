    ...
    <div className="min-h-screen bg-[#070913] text-white p-3 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-cyan-950/80 via-blue-950/60 to-[#070913] p-4 sm:p-6 rounded-2xl border border-cyan-500/30 shadow-[0_0_20px_rgba(0,243,255,0.1)]">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div className="hidden lg:block bg-slate-900/90 border border-cyan-500/30 p-2.5 rounded-xl text-right shrink-0">
            <span className="text-[9px] text-slate-400 font-bold block uppercase font-mono">Render Farm</span>
            <span className="text-xs font-extrabold text-cyan-400 font-['Creato_Display']">24 / 24 Active</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex overflow-x-auto gap-2 border-b border-cyan-500/20 pb-3 scrollbar-none no-scrollbar">
        {[
          ...
          { id: 'portfolio', label: `Portfolio & Files (${projects.length})`, icon: Video },
          { id: 'employees', label: `Team Directory (${EMPLOYEES.length})`, icon: Users },
          { id: 'projects', label: `Pipeline (${CLIENT_PROJECTS_PIPELINE.length})`, icon: FolderKanban },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div className="neon-card p-4 sm:p-5 border-cyan-500/30 space-y-2">
              <div className="flex items-center justify-between text-cyan-400">
                <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase">Monthly Revenue</span>
            ...
            <div className="neon-card p-4 sm:p-5 border-blue-500/30 space-y-2">
              <div className="flex items-center justify-between text-blue-400">
                <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase">Active Sprints</span>
                <FolderKanban className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <p className="text-xl sm:text-2xl font-extrabold text-white font-['Creato_Display']">{CLIENT_PROJECTS_PIPELINE.length} Sprints</p>
              <p className="text-[11px] text-cyan-300 font-semibold">2 Projects in Final Review</p>
            </div>

            <div className="neon-card p-4 sm:p-5 border-purple-500/30 space-y-2">
              <div className="flex items-center justify-between text-purple-400">
                <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase">Employee Roster</span>
                <Users className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <p className="text-xl sm:text-2xl font-extrabold text-white font-['Creato_Display']">{EMPLOYEES.length} Specialists</p>
              <p className="text-[11px] text-slate-300 font-semibold">Average Workload: 82%</p>
            </div>
            ...
            <div className="neon-card p-4 sm:p-5 border-cyan-400/30 space-y-2 cursor-pointer hover:border-cyan-300 transition-colors" onClick={() => setActiveTab('portfolio')}>
              <div className="flex items-center justify-between text-cyan-300">
                <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase">Live Portfolio Items</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
            ...
            <div className="lg:col-span-7 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-['Creato_Display'] font-bold text-base sm:text-lg text-white">Active Animation & Video Sprints</h3>
                  ...
                  <div key={proj.id} className="neon-card p-4 border-cyan-500/20 space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        ...
                        <h4 className="font-bold text-xs sm:text-sm text-white">{proj.name}</h4>
                        <p className="text-[11px] text-slate-400">Client: {proj.client} • Lead: <strong className="text-cyan-300">{proj.lead}</strong></p>
                      </div>
                      <div className="text-right shrink-0">
                        <span className="text-xs font-extrabold text-cyan-300 block">{proj.budget}</span>
                        <span className="text-[10px] sm:text-[11px] text-yellow-400 font-semibold">{proj.status}</span>
                      </div>
                    </div>
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="neon-card p-4 sm:p-6 border-blue-500/30 space-y-4">
                <h3 className="font-['Creato_Display'] text-sm sm:text-base font-bold text-white">Team Availability</h3>
